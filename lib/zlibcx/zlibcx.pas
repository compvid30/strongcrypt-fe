...
uses ZDeflatecx, ZInflatecx, ZUtilcx;

{Transfer to corresponding functions in ZUtil}
procedure zmemcpy(destp: pBytef; sourcep: pBytef; len: uInt);
begin
  ZUtilcx.zmemcpy(destp, sourcep, len);
end;

{---------------------------------------------------------------------------}
function  zmemcmp(s1p, s2p: pBytef; len: uInt): int;
begin
  zmemcmp := ZUtilcx.zmemcmp(s1p, s2p, len);
end;

{---------------------------------------------------------------------------}
procedure zmemzero(destp: pBytef; len: uInt);
begin
  ZUtilcx.zmemzero(destp, len);
end;

{---------------------------------------------------------------------------}
procedure zcfree(opaque: voidpf; ptr: voidpf);
begin
  ZUtilcx.zcfree(opaque, ptr);
end;

{---------------------------------------------------------------------------}
function zcalloc(opaque: voidpf; items: uInt; size: uInt): voidpf;
begin
  zcalloc := ZUtilcx.zcalloc(opaque, items, size);
end;
...
function deflateInit(var strm: z_stream; level: int): int;
begin
  deflateInit := ZDeflatecx.deflateInit(strm,level);
end;
...
function deflate(var strm: z_stream; flush: int): int;
begin
  deflate := ZDeflatecx.deflate(strm, flush);
end;
...
function deflateEnd(var strm: z_stream): int;
begin
  deflateEnd := ZDeflatecx.deflateEnd(strm);
end;
...
function deflateInit2(var strm: z_stream; level, method, windowBits, memLevel, strategy: int): int;
begin
  deflateInit2 := ZDeflatecx.deflateInit2(strm, level, method, windowBits, memLevel, strategy);
end;
...
function deflateSetDictionary(var strm: z_stream; dictionary: pBytef; dictLength: uint): int;
begin
  deflateSetDictionary := ZDeflatecx.deflateSetDictionary(strm, dictionary, dictLength);
end;
...
function deflateCopy(dest: z_streamp; source: z_streamp): int;
begin
  deflateCopy := ZDeflatecx.deflateCopy(dest, source);
end;
...
function deflateReset(var strm: z_stream): int;
begin
  deflateReset := ZDeflatecx.deflateReset(strm);
end;
...
function deflateParams(var strm: z_stream; level: int; strategy: int): int;
begin
  deflateParams := ZDeflatecx.deflateParams(strm, level, strategy);
end;
...
function inflateInit(var z: z_stream): int;
begin
  inflateInit := ZInflatecx.inflateInit(z);
end;
...
function inflateInit_(z: z_streamp; const version: str255; stream_size: int): int;
begin
  inflateInit_ := ZInflatecx.inflateInit_(z, version, stream_size);
end;
...
function inflateInit2_(var z: z_stream; w: int; const version: str255; stream_size: int): int;
begin
  inflateInit2_ := ZInflatecx.inflateInit2_(z, w, version, stream_size);
end;
...
function inflateInit2(var z: z_stream; windowBits: int): int;
begin
  inflateInit2 := ZInflatecx.inflateInit2(z, windowBits);
end;
...
function inflateEnd(var z: z_stream): int;
begin
  inflateEnd := ZInflatecx.inflateEnd(z);
end;
...
function inflateReset(var z: z_stream): int;
begin
  inflateReset :=  ZInflatecx.inflateReset(z);
end;
...
function inflate(var z: z_stream; f: int): int;
begin
  inflate := ZInflatecx.inflate(z, f);
end;
...
function inflateSetDictionary(var z: z_stream; dictionary: pBytef; dictLength: uInt): int;
begin
  inflateSetDictionary := ZInflatecx.inflateSetDictionary(z, dictionary, dictLength);
end;
...
function inflateSync(var z: z_stream): int;
begin
  inflateSync := ZInflatecx.inflateSync(z);
end;
...
function inflateSyncPoint(var z: z_stream): int;
begin
  inflateSyncPoint := ZInflatecx.inflateSyncPoint(z);
end;
...
begin
  ...
  err := ZDeflatecx.deflateInit(stream, level);
  if err<>Z_OK then begin
    compress2 := err;
    exit;
  end;

  err := ZDeflatecx.deflate(stream, Z_FINISH);
  if err<>Z_STREAM_END then begin
    ZDeflatecx.deflateEnd(stream);
    if err=Z_OK then compress2 := Z_BUF_ERROR else compress2 := err;
    exit;
  end;
  destLen := stream.total_out;

  err := ZDeflatecx.deflateEnd(stream);
  compress2 := err;
end;
...
begin
  ...
  err := ZInflatecx.inflateInit(stream);
  if err<>Z_OK then begin
    uncompress := err;
    exit;
  end;

  err := ZInflatecx.inflate(stream, Z_FINISH);
  if err<>Z_STREAM_END then begin
    ZInflatecx.inflateEnd(stream);
    if err=Z_OK then uncompress := Z_BUF_ERROR else uncompress := err;
    exit;
  end;
  destLen := stream.total_out;

  err := ZInflatecx.inflateEnd(stream);
  uncompress := err;
end;
